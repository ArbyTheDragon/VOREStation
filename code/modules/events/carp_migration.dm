...
/datum/event/carp_migration/proc/spawn_fish(var/num_groups, var/group_size_min=3, var/group_size_max=5)
	...
	while (i <= num_groups)
		...
		for (var/j = 1, j <= group_size, j++)
			spawned_carp.Add(new /mob/living/simple_animal/hostile/carp/vore(spawn_locations[i])) // Vorestation edit
		i++

/datum/event/carp_migration/end()
	for(var/mob/living/simple_animal/hostile/carp/vore/C in spawned_carp) // Vorestation edit
		if(!C.stat)
