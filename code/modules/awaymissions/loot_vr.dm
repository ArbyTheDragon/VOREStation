...
/obj/effect/landmark/loot_spawn/New()
	...
	25;"viscerator", \
		...
		if("alien")
			...
			if(prob(33))
				if(live_cargo) // Carp! Since Facehuggers got removed.
					var/num = rand(1,3)
					for(var/i=0,i<num,i++)
						new /mob/living/simple_animal/hostile/carp/vore(C)
				else // Just a costume.
					new /obj/item/clothing/suit/storage/hooded/carp_costume(C)
			else if(prob(50))
				if(live_cargo) // Something else very much alive and angry.
					var/spawn_type = pick(/mob/living/simple_animal/hostile/alien/vore, /mob/living/simple_animal/hostile/alien/drone/vore, /mob/living/simple_animal/hostile/alien/sentinel/vore)
					new spawn_type(C)
				else // Just a costume.
		...
		if("mimic")
			...
			var/mob/living/simple_animal/hostile/mimic/crate/vore/mimic = new(src.loc)
